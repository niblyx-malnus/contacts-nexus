...
++  style
  ...
  ;:  weld
    ...
    " .container \{ max-width: 640px; margin: 0 auto; padding: 1.5rem; height: 100%; display: flex; flex-direction: column; }"
    " #contacts-list \{ flex: 1; min-height: 0; overflow-y: auto; padding-right: 4px; }"
    " #contacts-list::-webkit-scrollbar \{ width: 8px; }"
    " #contacts-list::-webkit-scrollbar-thumb \{ background: #2e2e32; border-radius: 4px; }"
    " #contacts-list::-webkit-scrollbar-thumb:hover \{ background: #4a4a50; }"
    " .header \{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }"
    " h1 \{ font-size: 1.5rem; font-weight: 700; color: #fff; }"
    " .btn \{ padding: 0.4rem 0.8rem; border: 1px solid #444; border-radius: 6px; background: none; color: #aaa; cursor: pointer; font-size: 0.85rem; display: inline-flex; align-items: center; gap: 0.35rem; font-family: inherit; }"
    " .btn:hover \{ color: #fff; border-color: #666; }"
    " .btn-save \{ background: #2f6fbf; color: #fff; border-color: #2f6fbf; }"
    " .btn-save:hover \{ background: #3d82d6; border-color: #3d82d6; }"
    " .btn-delete \{ color: #f87171; border-color: #7a3535; }"
    " .btn-delete:hover \{ background: rgba(248,113,113,0.12); color: #f87171; }"
    " .btn-add \{ width: 36px; height: 36px; border-radius: 50%; border: 1.5px solid #3a3a40; background: none; color: #8a8a92; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s; }"
    " .btn-add:hover \{ border-color: #2f6fbf; color: #fff; }"
    " .btn-close \{ background: none; border: none; cursor: pointer; color: #8a8a92; display: flex; padding: 2px; border-radius: 4px; }"
    " .btn-close:hover \{ color: #fff; background: #26262a; }"
    " .contact-card \{ display: flex; align-items: center; background: #1c1c1e; border: 1px solid #2e2e32; border-radius: 10px; margin-bottom: 0.5rem; transition: border-color 0.15s; }"
    " .contact-card:hover \{ border-color: #4a4a50; }"
    " .card-main \{ display: flex; gap: 0.75rem; padding: 0.75rem; flex: 1; cursor: pointer; min-width: 0; }"
    " .card-delete \{ background: none; border: none; color: #55555c; cursor: pointer; padding: 0.75rem; display: flex; align-items: center; border-radius: 0 10px 10px 0; visibility: hidden; }"
    " .contact-card:hover .card-delete \{ visibility: visible; }"
    ...
    " .name \{ font-weight: 600; font-size: 0.9rem; color: #fff; }"
    " .ship-sub \{ font-size: 0.7rem; color: #8a8a92; font-family: monospace; display: flex; align-items: center; gap: 0.4rem; }"
    " .ames-tag \{ font-size: 0.6rem; padding: 0 4px; border-radius: 3px; font-family: sans-serif; }"
    " .ames-tag.known \{ background: #12332e; color: #6fcabc; }"
    " .ames-tag.alien \{ background: #33290f; color: #d9b25f; }"
    " .card-tags \{ display: flex; gap: 0.3rem; flex-wrap: wrap; }"
    " .card-tag \{ font-size: 0.65rem; padding: 1px 6px; border-radius: 3px; background: #26262a; color: #a8a8b0; }"
    " .tags-container \{ display: flex; gap: 0.3rem; flex-wrap: wrap; margin-bottom: 0.3rem; }"
    " .tag-chip \{ display: inline-flex; align-items: center; gap: 2px; font-size: 0.8rem; padding: 2px 8px; border-radius: 4px; background: #26262a; color: #d8d8dd; }"
    " .tag-chip button \{ background: none; border: none; cursor: pointer; color: #8a8a92; font-size: 1rem; padding: 0 2px; line-height: 1; }"
    " .tag-chip button:hover \{ color: #f87171; }"
    " .tags-input-wrap \{ margin-top: 0.2rem; }"
    " .status \{ font-size: 0.8rem; color: #a8a8b0; }"
    " .muted \{ color: #8a8a92; }"
    ...
    " .modal-content \{ background: #1c1c1e; border: 1px solid #3a3a40; border-radius: 12px; padding: 1.5rem; width: 90%; max-width: 480px; }"
    " .modal-header \{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem; }"
    " .modal-header h2 \{ font-size: 1.1rem; }"
    " .modal-tabs \{ display: flex; gap: 0; margin-bottom: 1rem; border-bottom: 2px solid #2e2e32; }"
    " .tab \{ padding: 0.4rem 0.75rem; border: none; background: none; cursor: pointer; font-size: 0.85rem; color: #8a8a92; border-bottom: 2px solid transparent; margin-bottom: -2px; font-family: inherit; }"
    " .tab:hover \{ color: #fff; }"
    " .tab.active \{ color: #fff; font-weight: 600; border-bottom-color: #2f6fbf; }"
    " .modal-body \{ display: flex; flex-direction: column; gap: 0.75rem; }"
    " .field-row \{ display: flex; flex-direction: column; gap: 0.25rem; }"
    " .field-row label \{ font-size: 0.8rem; font-weight: 500; color: #a8a8b0; }"
    " .field-input \{ padding: 0.5rem; border: 1px solid #3a3a40; border-radius: 6px; font-size: 0.9rem; background: #131315; color: #eee; font-family: inherit; }"
    " .field-input:focus \{ outline: none; border-color: #2f6fbf; }"
    ...
    " .profile-label \{ font-size: 0.7rem; color: #8a8a92; text-transform: uppercase; letter-spacing: 0.05em; }"
    " .profile-value \{ font-size: 0.9rem; color: #e2e2e6; }"
    " .profile-empty \{ color: #55555c; font-style: italic; font-size: 0.85rem; }"
  ==
::
++  js
  ^-  tape
